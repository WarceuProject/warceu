...
class New_Customers extends Report
{
    ...
    private function retunRowsForWeek()
    {
        ...
        $sql = "SELECT count(u.`id`) as count, DATE_FORMAT(u.`dateActivated`, '%W') as day, DATE_FORMAT(u.`dateActivated`, '%w') as day_num, DATE_FORMAT(u.`dateActivated`, '%Y-%m-%d') as dateActivated, DATE_FORMAT(u.`dateActivated`, '%d') as dayActivated FROM `users` u WHERE u.`groupid` IN (SELECT g.`id` FROM `groups` g WHERE g.`isadmin` = 0) AND u.`id` IN (SELECT DISTINCT i.`customerid` FROM `invoice` i WHERE i.`status` = ?) AND DATE_ADD(dateActivated, INTERVAL 7 DAY) > NOW() GROUP BY dateActivated ORDER BY u.`dateActivated` ASC ";
        $result = $this->db->query($sql, INVOICE_STATUS_PAID);
    ...
    private function returnRows($option_id, $currentYear)
    {
        ...
        $aYears = array();
        $sql = "SELECT COUNT(u.`id`) as usercount, MONTH(u.`dateActivated`) as monthactivated, YEAR(u.`dateActivated`) as yearactivated, DATE_FORMAT(u.`dateActivated`, '%Y-%m-1') as dateActivated FROM `users` u WHERE u.`groupid` IN (SELECT g.`id` FROM `groups` g WHERE g.`isadmin` = 0) AND u.`id` IN (SELECT DISTINCT i.`customerid` FROM `invoice` i WHERE i.`status` = ?) GROUP BY yearactivated, monthactivated ORDER BY yearactivated DESC ";
        $result = $this->db->query($sql, INVOICE_STATUS_PAID);
