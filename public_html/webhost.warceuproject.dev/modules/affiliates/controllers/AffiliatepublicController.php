...
class Affiliates_AffiliatepublicController extends CE_Controller_Action
{
    ...
    public function overviewAction()
    {
        ...
        if ($this->settings->get('Default Commission Structure') == '0') {
            ...
            $rate = $affiliateGateway->formatAffiliateCurrency($rate);
        }
        $this->view->commRate = $rate;
        $this->view->minimumPay = $affiliateGateway->formatAffiliateCurrency(
            $this->settings->get('Minimum Withdrawal')
        );

        $this->view->balance = $affiliateGateway->formatAffiliateCurrency($affiliate->balance);
        ...
        $this->view->totalCommissions = $affiliateGateway->formatAffiliateCurrency(
            $affiliateGateway->getPaidAffiliateCommission($affiliateId)
        );
        $this->view->pendingCommission = $affiliateGateway->formatAffiliateCurrency(
            $affiliateGateway->getPendingAffiliateCommission($affiliateId)
        );

        $this->view->showWithdrawlButton = ($affiliate->balance > $this->settings->get('Minimum Withdrawal')) ? true : false;
    }
    ...
    public function getcommissionsAction()
    {
        ...
        while ($commission = $iterator->fetch()) {
            ...
            $a_commission['amount'] = $affiliateGateway->formatAffiliateCurrency($a_commission['amount']);
    ...
    public function requestwithdrawlAction()
    {
        ...
        if ($affiliate->balance < $this->settings->get('Minimum Withdrawal')) {
            $this->error = 1;
            $this->message = $this->user->lang(
                'Sorry, %s is the minimum Withdrawal amount.',
                $affiliateGateway->formatAffiliateCurrency($this->settings->get('Minimum Withdrawal'))
            );
            $this->send();
            return;
        }
        ...
        $message = 'Affiliate Account Withdrawal Request: ' . $affiliateGateway->formatAffiliateCurrency($affiliate->balance);
    ...
    public function registerAction()
    {
        ...
        if ($this->settings->get('Bonus Deposit') > 0) {
            $this->view->bonus = $affiliateGateway->formatAffiliateCurrency($this->settings->get('Bonus Deposit'));
        }
        ...
        if ($this->settings->get('Default Commission Structure') == '0') {
            ...
            $commission = $affiliateGateway->formatAffiliateCurrency($commission);
        }
